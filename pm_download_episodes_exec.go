...
func executeEpisodeDownloads(client *ABSClient, item PodcastItem, episodesToDownload []FeedEpisode, reasons []string, podcastTitle string, itemID string, noWait, dryRun, oldest, forceNewOnly, verbose bool, keep *int, quiet bool) int {
	if len(episodesToDownload) > 0 {
		sortAndPrintSelectedEpisodes(episodesToDownload, podcastTitle, reasons, oldest, verbose, quiet)
		queueAndTrackDownloads(client, item, episodesToDownload, itemID, noWait, dryRun, quiet)
...
func sortAndPrintSelectedEpisodes(episodesToDownload []FeedEpisode, podcastTitle string, reasons []string, oldest, verbose, quiet bool) {
	if !quiet {
		fmt.Print("\r\x1b[K")
...
func queueAndTrackDownloads(client *ABSClient, item PodcastItem, episodesToDownload []FeedEpisode, itemID string, noWait, dryRun, quiet bool) {
	if dryRun {
		if !quiet {
	...
	if !noWait {
		waitForEpisodeDownloads(client, item, itemID, quiet)
	}
}

func waitForEpisodeDownloads(client *ABSClient, item PodcastItem, itemID string, quiet bool) {
	if !quiet {
