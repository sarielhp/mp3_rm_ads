...
func showOPMLExportUsage() {
	fmt.Println("Usage: abs sync opml export <file> [options]")
	fmt.Println()
	fmt.Println("Export podcast RSS feeds provided by the server into an OPML XML file.")
	fmt.Println()
	fmt.Println("Details:")
	fmt.Println("  - Queries the podcast server (Audiobookshelf or PodFetch) for all hosted podcasts.")
	fmt.Println("  - For each podcast, retrieves or opens the public RSS feed URL.")
	fmt.Println("  - Generates a standard OPML 2.0 XML document containing all server podcast feeds.")
	fmt.Println("  - Writes the resulting OPML document to the specified <file> path.")
	fmt.Println("  - In turn, this OPML file can be imported into podcast apps (such as AntennaPod)")
	fmt.Println("    to subscribe to all podcasts on your server in one swoop.")
	fmt.Println()
	fmt.Println("Importing into AntennaPod:")
	fmt.Println("  1. Transfer the exported .opml file to your device.")
	fmt.Println("  2. Open AntennaPod -> Subscriptions -> menu (⋮) -> 'Import/Export'.")
	fmt.Println("  3. Select 'OPML import' and choose your exported file.")
	fmt.Println()
	...
	fmt.Println("  abs sync opml export podcasts.opml")
	fmt.Println("  abs sync opml export ~/Downloads/podcasts.opml --verbose")
	fmt.Println()
}
