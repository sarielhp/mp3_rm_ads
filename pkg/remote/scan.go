...
func transcribeAndDetectAdsRemoteScan(audioFile string, origDuration float64, cfg *types.Config, profile types.LLMProfile, quiet, verbose bool) (*types.TranscriptionData, []types.AdSegment, error) {
	...
	transData, err := pipeline.LoadOrTranscribe(audioFile, transcriptJSON, *cfg, opts, profile, origDuration, speedFactor, cfg.WhisperLanguage, cfg.WhisperPrompt, map[string]string{}, &isNewlyTranscribed, &t0)
	if err != nil {
		return nil, nil, err
	}
