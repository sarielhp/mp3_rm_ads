...
func RunRemoteStatus(cfg *types.Config, host string, transport RemoteTransport, quiet, verbose bool) error {
	...
	status := types.RemoteServerStatus{
		Host:      targetHost,
...
func FetchRemoteActiveTask(targetHost, remoteWorkDir string, transport RemoteTransport, cfg *types.Config, status *types.RemoteServerStatus) {
	...
	if strings.TrimSpace(statContent) != "" {
		var activeSt types.EpisodeStatusFile
		if json.Unmarshal([]byte(statContent), &activeSt) == nil {
			if activeSt.Original.DurationSec > 0 {
				status.ActiveDuration = format.FormatClock(activeSt.Original.DurationSec)
			}
			status.ActiveStage = activeSt.CurrentStep
			if status.ActiveStage == "" {
				if activeSt.Status == types.StateTranscribingRemotely {
					status.ActiveStage = "Step 1/3: Whisper Transcription"
				} else if activeSt.Status == types.StateCuttingRemotely {
			...
			if startTimeStr != "" {
				if tStart, err := time.Parse(time.RFC3339, startTimeStr); err == nil {
					...
					status.ActiveElapsed = format.FormatClock(elapsed.Seconds())
					if activeSt.Status == types.StateTranscribingRemotely && activeSt.Original.DurationSec > 0 {
						...
						if estTotalSec > 0 {
							...
							status.ActiveETA = fmt.Sprintf("Est. Remaining: ~%s (%d%%)", format.FormatClock(remSec), pct)
						}
					}
				}
			}
	...
	if status.ActiveDuration == "" && cfg != nil && cfg.PodcastsDir != "" {
		...
		if dur := GetEpisodeDurationForQueue(localAudio); dur > 0 {
			status.ActiveDuration = format.FormatClock(dur)
		}
	}
}

func FetchAndSortRemoteQueuedTasks(targetHost, remoteWorkDir string, transport RemoteTransport, cfg *types.Config, status *types.RemoteServerStatus) {
	...
	for _, qPath := range util.SplitLines(strings.TrimSpace(queuedOut)) {
	...
	sort.SliceStable(status.QueuedTasks, func(i, j int) bool {
		ti, tj := status.QueuedTasks[i], status.QueuedTasks[j]
		if pi, pj := priMap[ti], priMap[tj]; pi != pj {
			return pi > pj
		}
		recI, recJ := recMap[ti], recMap[tj]
		if recI != recJ {
			return recI && !recJ
		}
		if recI {
			pubI, pubJ := pubMap[ti], pubMap[tj]
			if !pubI.Equal(pubJ) {
				return pubI.After(pubJ)
			}
			di, dj := durMap[ti], durMap[tj]
			if di != dj {
				return di < dj
			}
			return ti < tj
...
func CheckAndRecoverRemoteWorker(targetHost, remoteWorkDir string, transport RemoteTransport, status *types.RemoteServerStatus) {
...
func FetchRemoteReadyEpisodesAndBatches(targetHost, remoteWorkDir string, transport RemoteTransport) ([]RemoteDoneItem, int, []types.RemoteBatchManifest) {
	var readyEpisodes []RemoteDoneItem
	var archiveCount int
	var batches []types.RemoteBatchManifest
	...
	if strings.TrimSpace(out) != "" {
		batchIDs := util.SplitLines(strings.TrimSpace(out))
...
func ResolveLocalAudioPath(podcastsDir, relTask string) string {
	...
	if fi, err := os.Stat(podDir); err == nil && fi.IsDir() {
		if entries, err := os.ReadDir(podDir); err == nil {
			basePrefix := util.StripExt(baseName)
