...
func TestPodFetchLibrariesAndPodcasts(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				"episodes": []podFetchEpisodeDTO{
					{
						...
						EpisodeID:       "ep-101",
						Name:            "Episode 101",
						URL:             "https://example.com/ep101.mp3",
						DateOfRecording: "2026-08-30",
						TotalTime:       3600,
						LocalURL:        "ep101.mp3",
						Description:     "Ep 101 description",
						Status:          "D",
					},
				},
			})
		case r.URL.Path == "/api/v1/podcasts" && r.Method == "POST":
			_ = json.NewEncoder(w).Encode(podFetchItemDTO{
				...
				Name:      "New Show",
				Directory: "new_show",
				RSSFeed:   "https://example.com/new.xml",
			})
		default:
	...
	if libs[0].MediaType != "podcast" {
		t.Errorf("expected podcast media type, got %s", libs[0].MediaType)
	}
	...
	if len(podcasts) != 1 || podcasts[0].ID != "1" || podcasts[0].Media.Metadata.Title != "Tech Show" {
		t.Fatalf("unexpected podcasts list: %+v", podcasts)
	}
...
func TestPodFetchEpisodesAndDownloads(t *testing.T) {
	...
	if err := be.DownloadEpisodes("1", eps); err != nil {
		t.Fatalf("DownloadEpisodes failed: %v", err)
	}
	if !downloadInvoked {
		t.Errorf("expected download endpoint to be invoked")
	}

	if err := be.DeletePodcastEpisode("1", "ep-55"); err != nil {
		t.Fatalf("DeletePodcastEpisode failed: %v", err)
	}
	if deletedEpisode != "ep-55" {
		t.Errorf("expected deleted episode ep-55, got %s", deletedEpisode)
	}
